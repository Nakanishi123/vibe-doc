...
逆引きはMarkdownへ保存しない。起動時に全対象文書を走査してメモリ上に生成する。

## 文書の更新

vibe-docはファイルシステムの常駐監視やポーリングを行わない。編集中の文書を反映したい利用者は、Web UIの更新操作を明示的に実行する。

更新操作は文書ツリーを再走査し、追加・変更・削除されたMarkdownファイルだけを再解析する。再解析後にメモリ上のインデックスと逆引きを更新して、画面へ最新の状態を表示する。
