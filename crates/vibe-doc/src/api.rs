...
use vibe_doc_core::parser::DocumentTree;

/// 起動時に構築し、全APIハンドラーで共有する文書スナップショット。
#[derive(Clone)]
...
async fn list_documents(
    ...
    let documents: Vec<&Document> = query
        ...
        .map_or_else(
            || state.snapshot.index.documents().collect(),
            |query| state.snapshot.index.search(query.trim()),
        );
    Json(
        documents
            .into_iter()
...
async fn list_tags(State(state): State<ApiState>) -> Json<Vec<TagSummary>> {
    Json(
        state
            ...
            .map(|tag| TagSummary {
                name: tag.to_owned(),
                count: state.snapshot.index.documents_with_tag(tag).len(),
            })
            .collect(),
    )
}
...
async fn get_lint(State(state): State<ApiState>) -> Json<LintResponse> {
    let errors = state
        ...
        .count();
    let warnings = state.snapshot.diagnostics.len() - errors;
    Json(LintResponse {
        errors,
        warnings,
        diagnostics: state
...
async fn get_next_index(
    ...
    let index = next_index(
        indexed_kind,
        state.snapshot.tree.documents.values().cloned(),
    );
...
#[cfg(test)]
mod tests {
    use super::{ApiState, DocumentQuery, list_documents};
    ...
    async fn document_list_searches_and_filters_the_in_memory_index() {
        ...
        let response = list_documents(
            State(ApiState::new(parse_document_tree(&root))),
            Query(DocumentQuery {
                q: Some("needle".into()),
                kind: Some("task".into()),
                ..Default::default()
